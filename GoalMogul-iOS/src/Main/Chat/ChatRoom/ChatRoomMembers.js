    ...
    return (
      <View style={styles.memberCardContainerStyle}>
        ...
        >
          ...
          />
          <Text
            style={{ fontSize: 18, marginLeft: 6, marginTop: 6, color: "#666" }}
          >
            {userDoc.name}
          </Text>
        </TouchableOpacity>
        <View style={styles.memberCardActionsStyle}>
          {isAdmin && memberStatus == "Member" && userId != userDoc._id && (
            <TouchableOpacity
              activeOpacity={0.6}
          ...
          {isAdmin && memberStatus == "Admin" && userId != userDoc._id && (
            <TouchableOpacity
              activeOpacity={0.6}
          ...
          {isAdmin && memberStatus == "JoinRequester" && userId != userDoc._id && (
            <TouchableOpacity
              activeOpacity={0.6}
              onPress={() => this.acceptJoinRequest(userDoc._id)}
            >
              <MaterialIcons name="check" size={27} color="#61BA68" />
            </TouchableOpacity>
          )}
          {isAdmin && userId != userDoc._id && (
            <TouchableOpacity
              ...
              style={{ marginLeft: 9 }}
            >
              <MaterialIcons name="cancel" size={27} color="#F47474" />
            </TouchableOpacity>
          )}
        </View>
      </View>
  ...
  if (chatRoom && chatRoom.roomType != "Direct") {
    isAdmin =
      ...
      chatRoom.members.find(
        (memberDoc) =>
          memberDoc.memberRef._id == userId && memberDoc.status == "Admin"
      );
  }
