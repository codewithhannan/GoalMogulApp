...
class ChatRoomConversation extends React.Component {
  ...
  renderAccessory(props, accessoryLocation) {
    ...
    return (
      ...
      >
        ...
        >
          <Text style={styles.attachedImageTextStyle}>Attached image</Text>
        </View>
  ...
  renderSendButton(props) {
    return (
      <Send {...props}>
        <View
          style={{ paddingRight: 15, paddingBottom: 15, position: "relative" }}
        >
          <Image
  ...
  async saveToCameraRoll(imageSource) {
    ...
    if (Platform.OS === "android") {
      FileSystem.downloadAsync(
        urlToRender,
        `${FileSystem.cacheDirectory}saveChatImage/${toHashCode(urlToRender)}`
      ).then((res) => {
...
const mapStateToProps = (state, props) => {
  ...
  if (chatRoom) {
    if (chatRoom.roomType == "Direct") {
      let otherUser =
        chatRoom.members &&
        chatRoom.members.find((memberDoc) => memberDoc.memberRef._id != userId);
      if (otherUser) {
