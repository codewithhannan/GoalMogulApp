    ...
    if (!title && item.isChatRoom) {
      title =
        item.roomType == "Direct"
          ? item.members.find(
              (memDoc) =>
                memDoc.memberRef._id.toString() != this.props.currentUserId
            )
          : item.name;
  ...
  renderCardContent(item) {
    ...
    return (
      <View style={{ justifyContent: "flex-start", flex: 1, marginLeft: 10 }}>
        ...
        <View
          style={{
            marginTop: item.roomType == "Group" && item.memberCount ? 3 : 6,
          }}
        >
          <Text
            style={{
  ...
  render() {
    ...
    if (item.isFriend || item.roomType == "Direct") {
      cardImage = item.isFriend
        ...
        : item.members &&
          item.members.find(
            (memDoc) => memDoc.memberRef._id != this.props.currentUserId
          );
      if (cardImage) {
