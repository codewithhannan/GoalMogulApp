...
class SearchOverlay extends Component {
  ...
  keyboardWillHide() {
    if (
      (!this.props.searchContent || this.props.searchContent.trim() === "") &&
  ...
  render() {
    const marginTop =
      Platform.OS === "ios" && IPHONE_MODELS.includes(DEVICE_MODEL) ? 45 : 70;

    return (
      <BaseOverlay verticalPercent={1} horizontalPercent={1} ref="baseOverlay">
        <MenuProvider customStyles={{ backdrop: styles.backdrop }}>
          <View
            style={{ ...styles.headerContainerStyle, paddingTop: marginTop }}
          >
            <SearchBar
