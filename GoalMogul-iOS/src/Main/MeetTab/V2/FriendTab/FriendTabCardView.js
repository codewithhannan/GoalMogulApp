...
class FriendTabCardView extends React.PureComponent {
    ...
    return [
      {
        ...
        onPress: () => {
          Alert.alert(`Remove ${name} as a friend`, undefined, [
  ...
  renderBottomSheet = (item) => {
    ...
    return (
      <BottomSheet ref={(r) => (this.bottomSheetRef = r)} buttons={options} />
    );
  };

  renderHeader(item) {
    return <UserCardHeader user={item} optionsOnPress={this.openOptionModal} />;
  }
