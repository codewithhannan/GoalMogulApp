...
class BottomSheet extends React.PureComponent {
    ...
    renderContent = () => {
        let items = this.props.buttons.map((item) => {
            const { image, text, onPress, textStyle, imageStyle, ...otherProps } = item;
            ...
            return (
                <DelayedButton 
                    onPress={onPress} 
                    key={text} 
                    style={{ backgroundColor: "white", flexDirection: "row", padding: 10, alignItems: "center" }}
                    {...otherProps} 
                >
                    {
                        image ? <Image source={image} style={[styles.defaultImageStyle, imageStyle]} /> : null
                    }
                    {/* <Image /> */}
