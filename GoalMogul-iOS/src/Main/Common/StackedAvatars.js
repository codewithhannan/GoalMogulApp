...
export const StackedAvatarsV2 = (props) => {
  ...
  if (participants) {
    ...
    const participantPictures = participants
      .filter(
        (participant) =>
          participant.rsvp === "Going" || participant.rsvp === "Interested"
      )
    ...
    return (
      <View
        style={{ ...styles.memberPicturesContainerStyle, width: picturesWidth }}
      >
        {pictures}
      </View>
    );
    ...
    const memberPictures = tribeMembers
      .filter(
        (member) => member.category === "Admin" || member.category === "Member"
      )
      .map((member, index) => {
        if (index > 1) return null;
