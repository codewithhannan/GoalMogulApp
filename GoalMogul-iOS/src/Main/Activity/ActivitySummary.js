...
class ActivitySummary extends React.Component {
  renderText(item) {
    ...
    const tribeText = belongsToTribe ? (
      <Text>
        <Text style={{ ...boldTextStyle }}> {belongsToTribe.name}</Text> Tribe
      </Text>
    ) : null;
    const eventText = belongsToEvent ? (
      <Text>
        <Text style={{ ...boldTextStyle }}> {belongsToEvent.title}</Text> Event
      </Text>
    ) : null;
    const actorText = (
...
const executeIfFunction = (f, values) =>
  f instanceof Function ? f(values) : f;
