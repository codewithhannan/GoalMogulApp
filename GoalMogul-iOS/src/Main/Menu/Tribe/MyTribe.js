...
class MyTribe extends React.PureComponent {
  ...
  handleStatusChange = (isMember, item) => {
    ...
    const statusActionSheet = actionSheet(requestOptions, cancelIndex, options);
    statusActionSheet();
  };
  ...
  handlePageSetting = (item) => {
    ...
    const tribeActionSheet = actionSheet(requestOptions, cancelIndex, options);
    tribeActionSheet();
  };
  ...
  renderTribeInfo(item) {
    ...
    return (
      <View style={styles.tribeInfoContainerStyle}>
        ...
        <Text style={{ ...styles.tribeSizeTextStyle }}>Created {date}</Text>
      </View>
    );
  }
  ...
  renderMemberTabs() {
    ...
    return (
      <View>
        ...
        <View
          style={{ height: 1, width: "100%", backgroundColor: "#DADADA" }}
        />
      ...
      <MenuProvider customStyles={{ backdrop: styles.backdrop }}>
        <View style={styles.containerStyle}>
          ...
          <FlatList
            ...
            onRefresh={() =>
              this.props.refreshMyTribeDetail(item._id, this.props.pageId)
            }
