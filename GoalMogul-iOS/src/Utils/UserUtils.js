...
export class MemberDocumentFetcher {
    ...
    static async getUserDocument(userId, token, maybeMemberMap, skipTransformUserForGiftedChat) {
        let result;
        if (MemberDocumentFetcher.cachedMemberMap[userId]) { // check cache
            ...
            result = (await API.get(`secure/user/profile?userId=${userId}`, token)).data;
        };
        if (result) { // cache the result
            MemberDocumentFetcher.cachedMemberMap[result._id] = result;
        };
        return skipTransformUserForGiftedChat? result : _transformUserForGiftedChat(result);
    };
};
MemberDocumentFetcher.cachedMemberMap = {};