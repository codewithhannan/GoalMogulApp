...
export const shareToChatSearch = (searchQuery, currentResults, limit, searchType) => (dispatch, getState) => {
...
export const shareToSelectedChats = (selectedItems, shareMessage, userToShare, chatRoomType) => (dispatch, getState) => {
    const { token } = getState().user;
    dispatch({
    ...
    (async () => {
        ...
        if (failedItems.length == 0) {
            Alert.alert('Success', 'User has been shared to the selected chat(s)');
            dispatch({
                type: SHARE_TO_CHAT_RESET,
            });
            Actions.pop();
            ...
            Alert.alert('Error', `Could not send message to some chats: ${failedChatRoomNames}`);
            dispatch({
                type: SHARE_TO_CHAT_UPDATE_SUBMITTING,
                payload: false,
            });
