...
export const refreshFriendsSearch = (query, pageSize) => (dispatch, getState) => {
    ...
    API.get(`secure/user/friendship/es?query=${query}&limit=${pageSize}&skip=0`, token).then(resp => {
...
export const loadMoreFriendsSearch = (query, pageSize, prevOffset) => (dispatch, getState) => {
    const { token } = getState().user;

    dispatch({ type: CHAT_NEW_LOAD_MORE_FRIENDS_SEARCH_BEGIN });

    API.get(`secure/user/friendship/es?query=${query}&limit=${pageSize}&skip=${prevOffset+pageSize}`, token).then(resp => {
        const results = resp.data;
        dispatch({
        ...
        Alert.alert('Error', 'Could not load more results. Please try again later.');
    });
};

export const createOrUpdateChatroom = (values, membersToAdd, chatId, isEdit, needsUpload) => (dispatch, getState) => {
    const { token } = getState().user;
	...
	const onError = (err) => {
		...
		console.log(`${DEBUG_KEY}: ${isEdit ? 'update' : 'create'} chat failed with err: `, err);
	};

    const imageUri = isEdit ? newChat.changes.picture : newChat.picture;
    if (needsUpload) {
        ImageUtils.getImageSize(imageUri).then(({ width, height }) => {
            // Resize image
            return ImageUtils.resizeImage(imageUri, width, height);
        }).then((image) => {
            // Upload image to S3 server
            return ImageUtils.getPresignedUrl(image.uri, token, (objectKey) => {}, 'ChatFile');
