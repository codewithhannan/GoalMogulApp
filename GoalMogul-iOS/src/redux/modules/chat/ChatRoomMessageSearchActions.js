import { CHAT_ROOM_SEARCH_MESSAGES_UPDATE_QUERY, CHAT_ROOM_SEARCH_MESSAGES_BEGIN, CHAT_ROOM_SEARCH_MESSAGES_CLEAR, CHAT_ROOM_SEARCH_MESSAGES, CHAT_ROOM_SEARCH_MESSAGES_UPDATE_PREVIEW } from "./ChatRoomReducers";
...
export const refreshChatMessageSearch = (chatRoomId, searchQuery, chatRoomMembersMap) => (dispatch, getState) => {
    ...
    MessageStorageService.searchMessagesInConversation(chatRoomId, searchQuery, async (err, results) => {
        const messages = results || [];
        let transformedMessages;
        try {
            transformedMessages = await Promise.all(messages.map(async messageDoc => {
                try {
                    messageDoc.creator = await MemberDocumentFetcher.getUserDocument(messageDoc.creator, token, chatRoomMembersMap);
                } catch(e) {
