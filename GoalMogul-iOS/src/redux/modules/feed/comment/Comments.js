...
export default (state = INITIAL_STATE, action) => {
    switch (action.type) {
        case COMMENT_LOAD: {
            ...
            const shouldUpdate = sanityCheckByPageId(newState, parentId, pageId, COMMENT_LOAD);
        ...
        case COMMENT_LOAD_DONE: {
            ...
            const shouldUpdate = sanityCheckByPageId(newState, parentId, pageId, action.type);
            if (!shouldUpdate) return newState;
            newState = _.set(newState, `${parentId}.${pageId}.loading`, false);
        ...
        case COMMENT_DELETE_SUCCESS: {
            ...
            newState = _.set(newState, `${parentRef}.transformedComments`, transformedComments);
        ...
        case GOAL_DETAIL_OPEN: {
            ...
            if (action.type === SHARE_DETAIL_OPEN || action.type === POST_DETAIL_OPEN) {
            ...
            if (!_.has(commentToUpdate, pageId)) {
                commentToUpdate = _.set(commentToUpdate, pageId, { ...INITIAL_COMMENT_PAGE });
            }
        ...
        case GOAL_DETAIL_CLOSE: {
            ...
            if (action.type === SHARE_DETAIL_CLOSE || action.type === POST_DETAIL_CLOSE) {
                entityId = action.payload.postId;
            }
...
const sanityCheck = (state, parentId, type) => {
    if (!_.has(state, parentId)) {
        console.warn(`${DEBUG_KEY}: [ ${type} ]: expecting parentId: ${parentId} but not found`);
        return false;
    }
    return true;
};
...
const sanityCheckByPageId = (state, parentId, pageId, type) => {
    if (!_.has(state, parentId)) {
        console.warn(`${DEBUG_KEY}: [ ${type} ]: expecting goalId: ${parentId} but not found`);
        return false;
    }
    if (!_.has(state, `${parentId}.${pageId}`)) {
        console.warn(`${DEBUG_KEY}: [ ${type} ]: expecting goalId: ${parentId} and ` + 
