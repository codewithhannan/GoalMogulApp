    ...
    switch (action.type) {
        ...
        case EVENT_DETAIL_OPEN: {
            ...
            if (pageId === undefined) {
                ...
                console.warn(`${DEBUG_KEY}: [ ${action.type} ] with pageId: ${pageId}`);
                return newState;
            }
            ...
            if (!_.has(eventObjectToUpdate, pageId)) {
                eventObjectToUpdate = _.set(eventObjectToUpdate, pageId, { ...INITIAL_EVENT_PAGE });
            }
            ...
            eventObjectToUpdate = _.set(eventObjectToUpdate, 'reference', reference);
        ...
        case EVENT_DETAIL_LOAD_SUCCESS: {
            ...
            eventObjectToUpdate = _.set(eventObjectToUpdate, `${pageId}.eventLoading`, false);
            eventObjectToUpdate = _.set(eventObjectToUpdate, 'reference', reference);
        ...
        case MYEVENT_SWITCH_TAB: {
            ...
            const shouldUpdate = sanityCheckPageId(newState, eventId, pageId, action.type);
            if (!shouldUpdate) return newState;

            let navigationState = _.get(newState, `${eventId}.${pageId}.navigationState`);
            navigationState = _.set(navigationState, 'index', index);
            newState = _.set(newState, `${eventId}.${pageId}.navigationState`, navigationState);
            newState = _.set(newState, `${eventId}.${pageId}.selectedTab`, navigationState.routes[index].key);
        ...
        case MYEVENT_MEMBER_SELECT_FILTER: {
            ...
            if (index || index === 0) {
                newState = _.set(newState, `${path}.memberNavigationState.index`, index);
            }
        ...
        case EVENT_UPDATE_RSVP_STATUS_SUCCESS: {
            ...
            eventToUpdate = _.set(eventToUpdate, 'participantCount', participantCount);
        ...
        case EVENTTAB_REFRESH_DONE: {
            ...
            data.forEach(event => {
                ...
                const hasPageReference = (oldReference !== undefined && 
                ...
                eventObjectToUpdate = _.set(eventObjectToUpdate, 'reference', newReference);
                newState = _.set(newState, `${eventId}`, eventObjectToUpdate);
            });
        ...
        case EVENT_FEED_FETCH: {
            ...
            const shouldUpdate = sanityCheckPageId(newState, eventId, pageId, action.type);
            if (shouldUpdate) {
                newState = _.set(newState, `${eventId}.${pageId}.feedLoading`, true);
            }
            return newState;
        }
        ...
        case EVENT_FEED_FETCH_DONE: {
            ...
            newState = _.set(newState, `${path}.feed`, _.uniq(oldData.concat(newData)));
...
const sanityCheckPageId = (state, entityId, pageId, type) => {
    ...
    const isPageIdInReference = entity.reference !== undefined && entity.reference.some(r => r === pageId);

    if (!_.has(entity, `${pageId}`)) {
        if (isPageIdInReference) {
            console.warn(`${DEBUG_KEY}: [ ${type} ]: pageId ${pageId} ` + 
