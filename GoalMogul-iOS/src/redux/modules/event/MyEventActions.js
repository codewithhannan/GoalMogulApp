 ...
 */
export const refreshMyEventDetail = (eventId, callback, pageId) => (dispatch, getState) => {
...
export const myEventSelectMembersFilter = (option, index, eventId, pageId) => (dispatch) => {
  dispatch({
 ...
 * input on type field
 */
export const refreshEventFeed = (eventId, dispatch, getState, callback, pageId) => {
  const { token } = getState().user;
