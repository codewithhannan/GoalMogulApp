...
class Tutorial extends React.Component {
  ...
  animate(extraAnimation = [], extraDuration = 0) {
    const sequenceAnimations = Animated.sequence([
      ...
      // Show challenge
      Animated.timing(this.challengeAnim, { duration: DURATION, toValue: 1 }),
      Animated.delay(PAUSE),
      Animated.timing(this.challengeAnim, { duration: DURATION, toValue: 0 }),
      // Show Learn
  ...
  animateHeader = (delay) => {
    ...
    const headerAnimation = Animated.sequence([
      ...
      Animated.timing(this.settingAccount, { duration: DURATION, toValue: 0 }),
      Animated.timing(this.setupComplete, { duration: DURATION, toValue: 0.8 }),
    ]);
  ...
  handleReplay = () => {
    ...
    this.animate(
      [Animated.timing(this.HostAnim, { duration: DURATION, toValue: 0 })],
      DURATION
    );
  };
  ...
  renderUnseenText = () => {
    return (
      ...
      >
        <Text style={{ color: "#124562", fontSize: 15 }}>We're done</Text>
        <Text style={{ color: "#124562", fontSize: 20, fontWeight: "600" }}>
          Set up complete!
        </Text>
      </Animated.View>
  ...
  renderHeader() {
    const paddingTop =
      Platform.OS === "ios" && IPHONE_MODELS.includes(DEVICE_MODEL) ? 30 : 50;
