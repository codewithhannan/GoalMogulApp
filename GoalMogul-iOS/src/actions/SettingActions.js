...
export const onVerifyPhoneNumber = (handleRedirect) => {
  return (dispatch, getState) => {
    const { token, userId } = getState().user;
    return API.post("secure/user/account/verification", { for: "phone" }, token)
      .then(async (res) => {
...
export const getBlockedUsers = (refresh) => (dispatch, getState) => {
  ...
  if (hasNextPage === undefined || hasNextPage) {
    API.get(`secure/user/settings/block?skip=${newSkip}&limit=${limit}`, token)
      .then((res) => {
...
export const saveNotificationSetting = (formValues) => (dispatch, getState) => {
  ...
  API.put(`${BASE_ROUTE}/notification-preferences`, { ...prefToUpdate }, token)
    .then((res) => {
      if (res.status === 200) {
        onSuccess(res);
        return;
