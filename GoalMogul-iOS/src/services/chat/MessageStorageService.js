...
class MessageStorageService {
                ...
                transformedDocs.forEach((messageDoc) =>
                  localDb.insert(messageDoc, (err) => {
                    ...
                    if (processedCount == transformedDocs.length) {
                      ...
                      callback(
                        null,
                        localDocs.concat(insertedDocs.slice(0, limit))
                      );
                    }
                  })
                );
      ...
      .exec((err, messagesAfter) => {
        ...
        localDb
          ...
          .exec((err, messagesBefore) => {
            ...
            callback(
              null,
              arrayUnique(messagesAfter.concat(messagesBefore)).sort(
                (doc1, doc2) => doc2.created - doc1.created
              )
            );
          });
      });
  ...
  searchMessagesInConversation = (conversationId, searchQuery, callback) => {
    if (
      searchQuery.length < FUSE_MESSAGE_SEARCH_OPTIONS.minMatchCharLength ||
