...
class SortableFlatList extends Component {
    ...
    constructor(props) {
        ...
        this._panResponder = PanResponder.create({
            onStartShouldSetPanResponderCapture: (evt, gestureState) => {
                ...
                if (Platform.OS === 'android' && !horizontal) {
                    const isTranslucent = StatusBar._propsStack.reduce(
                        (acc, cur) => {
                            return cur.translucent === undefined
                                ? acc
                                : cur.translucent
                        },
                        false
            ...
            onPanResponderRelease: () => {
                ...
                const lastElementMeasurements = this._measurements[
                    data.length - 1
                ]
    ...
    animate = () => {
        ...
        if (this._measurements[activeRow]) {
            ...
            const fingerPosition = Math.max(
                0,
                this._move - this._containerOffset
            )
            const shouldScrollUp =
                !isFirstItem &&
                fingerPosition < this._containerSize * scrollRatio
            const shouldScrollDown =
                !isLastItem &&
                fingerPosition > this._containerSize * (1 - scrollRatio)
    ...
    getSpacerIndex = (move, activeRow) => {
        ...
        const hoverItemMidpoint =
            move - this._additionalOffset + hoverItemSize / 2
        ...
        if (spacerIndex === undefined) {
            ...
            spacerIndex = this._measurements.findIndex(
                ({ width, height, x, y }) => {
                    const itemOffset = horizontal ? x : y
                    const itemSize = horizontal ? width : height
                    return (
                        hoverPoint > itemOffset &&
                        hoverPoint < itemOffset + itemSize
                    )
                }
            )
        }
        ...
        !!this._refs[index] &&
            setTimeout(() => {
                try {
                    ...
                    !!this._refs[index] &&
                        this._refs[index].measureInWindow(
                            (x, y, width, height) => {
...
class RowItem extends PureComponent {
    renderSpacer = (size) => (
        <View
            style={this.props.horizontal ? { width: size } : { height: size }}
        />
    )
            ...
            >
                ...
                <View
                    style={[
                        ...
                        { opacity: isActiveRow ? 0.4 : 1, overflow: 'hidden' },
                    ]}
                >
                    {component}
                </View>
