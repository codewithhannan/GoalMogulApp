 ...
 *    Also the <KeyboardAvoidingView> that wraps {@var fragment}, has an added style
 *      > display = 'flex', flexGrow = 1
 * 2. In {@function render}, the main <View> that wraps {@function renderMessages} and {@function renderInputToolbar}
 *    recieves an added style field on top of {@var styles.container} as:
 *      > paddingBottom = this.getKeyboardHeight() - this.getBottomOffset()
...
import { IMessage, LeftRightStyle, Reply, User } from 'react-native-gifted-chat'
...
import { DATE_FORMAT, DEFAULT_PLACEHOLDER, MAX_COMPOSER_HEIGHT, MIN_COMPOSER_HEIGHT, TIME_FORMAT } from 'react-native-gifted-chat/lib/Constant'
...
class GMGiftedChat<TMessage extends IMessage = IMessage> extends React.Component<
  ...
  renderMessages() {
    ...
    return this.props.isKeyboardInternallyHandled ? (
      <KeyboardAvoidingView style={{
        display: 'flex',
        flexGrow: 1,
      }} enabled>{fragment}</KeyboardAvoidingView>
    ) : (
      fragment
    )
  }
